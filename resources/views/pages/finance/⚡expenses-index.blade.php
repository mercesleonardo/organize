...
new #[Title('Expenses')] class extends Component {
    ...
    public string $edit_type = 'expense';
    ...
    public function openEdit(int $transactionId): void
    {
        ...
        $this->edit_type = TransactionType::Expense->value;
    ...
    public function saveEdit(): void
    {
        ...
        if ($category->type !== TransactionType::Expense) {
            throw ValidationException::withMessages([
                'edit_category_id' => __('Please choose an expense category.'),
            ]);
        }

        $newStatus = TransactionStatus::from($this->edit_status);
        $newAmount = (float) number_format((float) $this->edit_amount, 2, '.', '');
    ...
    <div class="flex flex-col gap-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <flux:text class="mt-1">{{ __('Your spending and card installments.') }}</flux:text>
            </div>
        ...
        <div class="overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700">
            ...
            <table class="hidden min-w-full divide-y divide-zinc-200 dark:divide-zinc-700 md:table">
                <thead class="bg-zinc-50 dark:bg-zinc-900/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-start text-xs font-medium uppercase text-zinc-500">{{ __('Payment') }}</th>
                ...
                <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-700 dark:bg-zinc-800">
                    @forelse ($this->transactions as $transaction)
                        <tr wire:key="exp-{{ $transaction->id }}">
                            ...
                            <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-300">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @if ($this->isInvestmentDebit($transaction))
                                        <flux:badge size="sm" inset="top bottom" color="emerald">
                                            {{ __('Investment') }}
                                        </flux:badge>
                                    @endif
                                </div>
                            ...
                            <td class="whitespace-nowrap px-4 py-3 text-end text-sm">
                                ...
                                <flux:button
                                    ...
                                    wire:confirm="{{ __('Delete this expense? If this is the master installment, the other installments will be removed too.') }}"
                                >
                                    {{ __('Delete') }}
                                </flux:button>
                            </td>
            ...
            <div class="divide-y divide-zinc-200 bg-white dark:divide-zinc-700 dark:bg-zinc-800 md:hidden">
                ...
                @empty
                    <x-empty-state :message="__('No expenses in this period.')" />
                @endforelse
            </div>
        </div>
        ...
        <form wire:submit="saveEdit" class="space-y-6">
            ...
            <flux:field>
                ...
                @if ($this->edit_category_id)
                    @php
                        $editExpenseCategory = $this->categoriesForEdit->firstWhere('id', $this->edit_category_id);
                    @endphp
            ...
            <flux:radio.group wire:model="edit_status" :label="__('Payment')">
